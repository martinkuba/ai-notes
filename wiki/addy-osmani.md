...
- **Owning the Outer Loop** — As agents take over the inner loop (investigate, implement, verify), engineers must own the outer loop: the accountability layer where humans render Verdicts on production changes. Three pillars: **Quality** (evidence-gathering checks), **Verdict** (the human decision to ship or block), and **Answerability** (the ability to explain a decision after the fact). Ties [cognitive surrender](#coined-concepts), [cognitive debt](cognitive-debt.md), and the [orchestration tax](orchestration-tax.md) together as the three hidden costs of delegation, and argues accountability and taste — qualitative judgment without metrics — are the scarce resources that scale agentic factories. See source: [Own the Outer Loop](../summaries/own-the-outer-loop.md).
