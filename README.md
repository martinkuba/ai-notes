...
```
sources/           Raw source documents (articles, papers, data)
                   Synced from Readwise Reader. LLM reads but never modifies.
...
- **Ingest** — After syncing new sources, ask Claude to ingest them. It will read the sources, create/update wiki pages, and maintain cross-references, the index, and the log.
