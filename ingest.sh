...
# 3. Run Claude to ingest new/unreferenced sources
echo "==> Running Claude to ingest sources into wiki..."
claude -p --permission-mode auto --allowedTools "Read Edit Write Glob Grep Bash" << 'PROMPT'
You are maintaining an AI research wiki. Your job is to ingest source documents into the wiki.

1. Find all source files in sources/*.md that are NOT yet referenced by any wiki page in wiki/*.md
   (search for their filename pattern like ../sources/filename.md across all wiki pages)

2. For each unreferenced source:
   ...
   - Use standard markdown links: [Display Title](../sources/filename.md) for sources, [Page Title](page-name.md) for wiki pages
...
Follow the conventions in CLAUDE.md. Do not modify source files.
PROMPT
